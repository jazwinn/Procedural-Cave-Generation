...
Mesh::Mesh(std::vector<GLfloat>& vboData, std::vector<GLuint>& eboData, GLuint instance, const std::vector<Instance>& instanceMatrix) :
	m_vertices{ vboData },
	m_indices{ eboData },
	m_instance{instance},
	m_VAO{},
	m_VBO(vboData),
	m_instanceVBO(instanceMatrix),
	m_EBO(eboData)
{
	...
	if (m_instance > 1 && !instanceMatrix.empty())
	{
		GLsizei instanceStride = sizeof(Instance);
		...
		m_VAO.LinkAttribute(m_instanceVBO, 5, 1, GL_FLOAT, instanceStride, (void*)(4 * sizeof(glm::vec4))); // coordinates
	...
	m_vertexCount = vboData.size() / stride;
